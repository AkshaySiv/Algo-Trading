...
class CapitalComAPI:
    ...
    def place_limit_order(self, epic: str, direction: str, size: float,
                          ...
                          profit_level: Optional[float] = None) -> dict:
        """
        Place a limit entry order (executes when price reaches 'level' favorably).
        BUY LIMIT: entry below current price. SELL LIMIT: entry above current price.
    ...
    def place_stop_order(self, epic: str, direction: str, size: float,
        ...
        BUY STOP: entry above current price (breakout long).
        SELL STOP: entry below current price (breakout short).

        Args:
            epic:         Instrument epic
            direction:    "BUY" or "SELL"
            size:         Position size
            level:        Breakout trigger price
            stop_level:   Stop-loss price
            profit_level: Take-profit price

        Returns:
            dict with dealReference (confirm with confirm_deal() to get dealId)
        """
        self._ensure_session()
        payload = {
            "epic": epic,
        ...
        if resp.status_code in (200, 201):
            result = resp.json()
            logger.info(f"Stop order placed: {direction} {size} {epic} @ {level} | "
